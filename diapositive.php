...
/*
Plugin Name: Aggiungi diapositive
Description: aggiunge diapositive in homepage
*/
...
function slide_post_class_meta_box( $object, $box ) { ?>
	...
	<?php wp_nonce_field( basename( __FILE__ ), 'slide_post_class_nonce' ); ?>

	<p>
		<label for="slide-post-class"><?php _e( "Select the content of the site that this slide should be connected.", 'slide_slideshow' ); ?></label>
		</p>
		<p>
       ...
       <?php // The Query
	   	$query_args = array(
				'post_type' => array('post', 'page', 'vini' , 'riconoscimento', 'tribe_events'),
				'posts_per_page' => -1
);
		...
		while ( $the_query->have_posts() ) :
			$the_query->the_post();
			echo '<option ';
			if(esc_attr( get_post_meta( $object->ID, 'slide_post_class', true ) ) == get_the_ID() ){ echo ' selected ';}
...
function slide_save_post_class_meta( $post_id, $post ) {
	...
	if ( !isset( $_POST['slide_post_class_nonce'] ) || !wp_verify_nonce( $_POST['slide_post_class_nonce'], basename( __FILE__ ) ) )
		return $post_id;
